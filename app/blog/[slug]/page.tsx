...
import { articleSchema, createPageMetadata, faqSchema, StructuredData } from "@/components/seo";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createPageMetadata({
    ...
    keywords: [article.keyword, "China Company Registration", "China Accounting", "China Tax Consultant", "China Business Registration", "Foreign Investment China"],
...
function articleSections(article: NonNullable<ReturnType<typeof getArticleBySlug>>) {
  return pillars.map((pillar, index) => ({
    ...
    body: `${article.title} should be evaluated through the lens of ${pillar}, because foreign investors rarely face a single isolated filing in China. A practical plan connects China Company Registration, China Accounting, China Tax Consultant input, China Business Registration, China Company Formation, Chinese Accounting Firm support, and Foreign Investment China decisions. For international SMEs, the most common issue is not only understanding the rule, but sequencing documents, approvals, reporting, banking, invoices, employment, and shareholder expectations in a way that management can actually execute. ZYS recommends documenting the commercial purpose, target city, ownership chain, contract model, hiring plan, expected revenue flow, overseas payment needs, and filing calendar before choosing the final implementation route. This reduces avoidable delays and creates a cleaner audit trail for tax, accounting, visa, business license, and annual compliance work.`
  }));
}
      ...
      <article>
        <header className="bg-white py-16 md:py-24">
          <div className="container-shell max-w-4xl">
            <p className="text-sm font-bold uppercase text-evergreen">{article.keyword}</p>
            ...
            <p className="mt-5 text-sm font-semibold text-graphite">By {article.author} | Published {article.published}</p>
          </div>
        </header>
